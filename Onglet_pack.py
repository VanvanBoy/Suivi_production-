# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 15:26:59 2025
...
def create_pack_interface(tab,conn,cursor):
    ...
    def etape_pack():
        ...
        if valeur_control=='Yes':
            ...
            etat=0
        if commentaire:
            query="UPDATE suivi_production set soudure_pack=%s, date_soudure_pack=%s,commentaire=%s where numero_serie_batterie=%s"
            param=(etat,date,commentaire,num_batt)
        else : 
            query="UPDATE suivi_production set soudure_pack=%s, date_soudure_pack=%s where numero_serie_batterie=%s"
            param=(etat,date,num_batt)
        if etat==1:
            messagebox.showinfo("Succès!","Point de controle soudure du pack OK! Passage à l'etape suivante")
        else : 
            messagebox.showerror("Fail!","Point de controle soudure du pack NON OK!")
    ...
    ctrl_label = tk.Label(right_frame, text="Controle soudure du pack:", font=('Arial', 12))
    ...
    # Bouton Sauvegarder
    submit_button = tk.Button(right_frame, text="Process", command=etape_pack, font=('Arial', 12), bg='blue', fg='white')
