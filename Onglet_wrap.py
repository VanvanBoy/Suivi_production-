# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 15:52:42 2025
...
def create_wrap_interface(tab,conn,cursor):
    ...
    def etape_wrap():
        ...
        if commentaire:
            query="UPDATE suivi_production set wrap=%s, date_wrap=%s,commentaire=%s where numero_serie_batterie=%s"
            param=(etat,date,commentaire,num_batt)
        else : 
            query="UPDATE suivi_production set wrap=%s, date_wrap=%s where numero_serie_batterie=%s"
            param=(etat,date,num_batt)
        if etat==1:
            messagebox.showinfo("Succès!","Point de controle wrap OK! Passage à l'etape suivante")
        else : 
            messagebox.showerror("Fail!","Point de controle wrap NON OK!")
    ...
    ctrl_label = tk.Label(right_frame, text="Controle wrap:", font=('Arial', 12))
    ...
    # Bouton Sauvegarder
    submit_button = tk.Button(right_frame, text="Process", command=etape_wrap, font=('Arial', 12), bg='blue', fg='white')
