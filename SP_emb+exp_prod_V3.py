...
class StockApp(ThemedTk):
    ...
    def setup_emb(self, frame):
        ...
        self.btn_emb_batt = ttk.Button(
            left_frame, text="Emballer les batteries",
    ...
    def emballer_batterie(self):
        ...
        for batt in batt_emballees:
            ...
            try:
                cursor = conn.cursor()
                query_v='select emballage from suivi_production where numero_serie_batterie = %s'
                param_v=(batt,)
                cursor.execute(query_v, param_v)
                row = cursor.fetchone()
                if row[0]==1: 
                    messagebox.showwarning("Deja emballée", f"Le N° série {batt} a déjà été emballé.")
    ...
    def emb_check_entry_length(self, event=None):
        ...
        try:
            cursor = conn.cursor()
            query_v='select emballage from suivi_production where numero_serie_batterie = %s'
            param_v=(numero_serie_cell,)
            cursor.execute(query_v, param_v)
            row = cursor.fetchone()
            if row[0]==1: 
                messagebox.showwarning("Deja emballée", f"Le N° série {numero_serie_cell} a déjà été emballé.")
                self.emb_numero_serie_batt_entry.delete(0, tk.END)
                return
