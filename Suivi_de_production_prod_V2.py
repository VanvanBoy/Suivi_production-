        ...
        try:
            ...
            if modele[:8]=='PPTR018A':
                ...
                if imp_min <= impedance <= imp_max :
                    query = "UPDATE suivi_production SET soudure_pack = 1, date_soudure_pack = NOW(),test_impedance= %s, date_impedance= NOW() where numero_serie_batterie = %s "
                    param = (impedance,num_batt)  
                    cursor.execute(query, param)
                    messagebox.showinfo("Controle OK",f"Batterie {num_batt} controlée")
                else :
