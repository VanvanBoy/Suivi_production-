...
class StockApp(ThemedTk):
    ...
    def verif_etape_act_non_ok(self, current_stage, num_prod):
        ...
        # Colonnes non présentes dans la BDD
        ignored_stages = {"recherche", "recyclage", "tri_test"}
    
        dbcol = self.STAGE_TO_DBCOL[current_stage]
    
        # On garde uniquement les colonnes réellement présentes en BDD
        valid_cols = [
            col for stage, col in self.STAGE_TO_DBCOL.items()
            if stage not in ignored_stages
        ]
        ...
        conn = self.db_manager.connect()
        cursor = conn.cursor(dictionary=True)
        cursor.execute(query, (num_prod,))
        row = cursor.fetchone()
    
        if not row:
            messagebox.showerror("Erreur", "Batterie introuvable")
            return False
    
        bool_stage = row.get(dbcol, 0)
    
        if bool_stage == 1:

            ignored_stages = {"recherche", "recyclage", "tri_test"}
        
            current_rank = self.stage_order[current_stage]
        
            # tri des étapes par ordre de production
            ordered_stages = sorted(self.stage_order.items(), key=lambda x: x[1])
        
            next_stage = None
        
            for stage, rank in ordered_stages:
                ...
                if stage not in self.ALLOWED_STAGE_KEYS:
                    continue
        
                dbcol = self.STAGE_TO_DBCOL.get(stage)
            ...
            messagebox.showerror(
                "Étape déjà validée",
                f"L'étape '{current_stage}' est déjà validée.\n\n"
                f"La batterie doit aller à l'étape : '{next_stage}'."
            )
    ...
    def show_of_in_process(self):
        conn = self.db_manager.connect()
        cursor = conn.cursor()
        query='select n_of from ref_of where etat_fabrication=%s'
        param=(0,)
        cursor.execute(query,param)
        result=cursor.fetchall()
        liste_lot = [row[0] for row in result]
        cursor.close()
        conn.close()
        return liste_lot
    ...
    def swap_of(self, numero_serie_initial, nouvel_of,ancien_of):
        ...
        try:
            ...
            # 🔢 Demande nouveau numéro de série
            ...
            # 🔍 Vérifier batterie remplacement
            query = "SELECT n_of FROM produit_voltr WHERE numero_serie_produit = %s"
    ...
    def replace_cellule(self):
        ...
        self.remplacement_dir = r"G:\Drive partagés\VoltR\4_Production\8_Picking\remplacement\Remplacement en cours"
    ...
    def update_avancement(self, stage, valeur,num_of):
        ...
        if widget is not None:
            widget.config(text=valeur)  # ou .set() selon ton widget
        else:
            print(f"Widget {widget_name} introuvable")
            
        if combo is not None:
            combo.set(num_of)  # ou .set() selon ton widget
        else:
    ...
    def of_avancement(self,event=None):
        ...
        try:
            ...
            query=f"SELECT quantite_batterie FROM ref_of where n_of=%s"
            param=(num_of,)
            cursor.execute(query,param)
            tot_of=cursor.fetchall()[0][0]
            
            query=f"SELECT count(pv.numero_serie_produit) FROM produit_voltr as pv join suivi_production as sp on sp.numero_serie_batterie=pv.numero_serie_produit where pv.n_of=%s and sp.{colonne}=1"
            param=(num_of,)
            cursor.execute(query,param)
            nb_of=cursor.fetchall()[0][0]
            
            ratio="Avancement of :"+"\n"+str(nb_of)+"/"+str(tot_of)   
    ...
    def setup_pack(self, frame):
        ...
        if str(self.selected_model)[:8]=='PPTR018A':
            if str(self.selected_model)=='PPTR018AA':
                ttk.Label(right_frame, text="Bosch",font=('Segoe UI', 23)).pack(pady=5)
            elif str(self.selected_model)=='PPTR018AB':
                ttk.Label(right_frame, text="Makita",font=('Segoe UI', 23)).pack(pady=5)
            elif str(self.selected_model)=='PPTR018AC':
                ttk.Label(right_frame, text="Ryobi",font=('Segoe UI', 23)).pack(pady=5)
    ...
    def valider_soudure_pack(self):
        ...
        try:
            ...
            if modele[:8]=='PPTR018A':
                ...
                try:
                    tension_min = float(seuils["tension_pack_min"])
                except Exception:
                    tension_min = float(str(seuils["tension_pack_min"]).replace(",", "."))
                try:
                    tension_max = float(seuils["tension_pack_max"])
                except Exception:
                    tension_max = float(str(seuils["tension_pack_max"]).replace(",", "."))
                
                if imp_min <= impedance <= imp_max and tension_min <= controle_tension <= tension_max :
                    ...
                    if not (imp_min <= impedance <= imp_max):
                        messagebox.showerror("Erreur impedance", "La valeure d'impedance est NOK")
                    if not (tension_min <= controle_tension <= tension_max) :
                        messagebox.showerror("Erreur tension", "La valeure de tension est NOK")   
                    return
    ...
    def _move_processed_files(self, ok_pairs, ko_files, dossier_exploites, dossier_ko):
        ...
        dossier_exp=os.path.join(dossier_exploites, model)
    ...
    def fl_check_entry_length(self, event=None):
        # (Optionnel) on ne déclenche que quand on a 12 caractères
        numero_serie_cell = self.fl_numero_serie_cell_entry.get().strip()
        if len(numero_serie_cell) != 12:
            return
        ...
        try:
            ...
            if row and row[0]:
                ...
                self.fl_numero_serie_batt_entry.insert(0, num_batt)
            else:
                # Efface si pas trouvé (optionnel)
                self.fl_numero_serie_batt_entry.delete(0, tk.END)
        finally:
    ...
    def fl_on_select_batt(self, event=None):
        """Quand on sélectionne une batterie dans la listbox, la mettre dans l'Entry produit."""
        selection = self.fl_listbox_batt.curselection()
        if not selection:
            return  # rien de sélectionné
        
        # Récupère le texte de la ligne sélectionnée
        selected_value = self.fl_listbox_batt.get(selection[0])
    
        # Mets à jour l'Entry produit
        self.fl_numero_serie_batt_entry.delete(0, tk.END)
        self.fl_numero_serie_batt_entry.insert(0, selected_value)
        self.of_avancement()
