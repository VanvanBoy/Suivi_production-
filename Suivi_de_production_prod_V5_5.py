...
class StockApp(ThemedTk):
    ...
    def replace_cellule(self):
        ...
        # Template 
        self.template_remplacement_path = r"G:\Drive partagés\11_Data\IHM\Executable\IHM_suivi_prod_beta\Suivi_prod_rsc\Template remplacement cellule (13).xlsx"
    ...
    def setup_nappe(self, frame):
        ...
        self.set_photo(
            self.n_label_photo,
            r"G:\Drive partagés\11_Data\IHM\Executable\IHM_suivi_prod_beta\Suivi_prod_rsc\voltr_logo.jpg"
        )
    ...
    def w_check_entry_length(self, event=None):
        # (Optionnel) on ne déclenche que quand on a 12 caractères
        numero_serie_cell = self.w_numero_serie_cell_entry.get().strip()
        if len(numero_serie_cell) != 12:
            return
        ...
        try:
            ...
            if row and row[0]:
                ...
                self.w_numero_serie_batt_entry.insert(0, num_batt)
            else:
                # Efface si pas trouvé (optionnel)
                self.w_numero_serie_batt_entry.delete(0, tk.END)
        finally:
