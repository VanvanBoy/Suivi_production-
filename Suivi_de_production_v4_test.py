# -*- coding: utf-8 -*-
"""
Created on Tue Aug 26 17:52:25 2025
...
class StockApp(ThemedTk):
    ...
    def display_model_list_fermeture(self):
        ...
        try: 
            cursor= conn.cursor()
            query = "SELECT numero_serie_batterie from suivi_production as sp join produit_voltr as pv on sp.numero_serie_batterie = pv.numero_serie_produit where sp.fermeture is null and pv.reference_produit_voltr = %s "
            param=(modele,)
            cursor.execute(query, param)
            rows = cursor.fetchall()  
    ...
    def valider_fermeture(self):
        ...
        try:
            cursor = conn.cursor()
            query = "UPDATE suivi_production SET fermeture = 1 where numero_serie_batterie = %s "
            param = (num_batt,)  
            cursor.execute(query, param)
        except Exception as e:
            messagebox.showerror("Erreur SQL", f"Impossible de récupérer les données :\n{e}")
    ...
    def setup_emb(self, frame):
        ...
        self.emb_numero_serie_cell_entry.bind("<KeyRelease>",self.emb_check_entry_length)
    ...
    def add_non_conf_batterie_emballage(self):
